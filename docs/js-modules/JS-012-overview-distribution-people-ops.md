...
| FN-078 | `computeRMScoreRows()` `#L513` | `leads` | per-RM `{open, breached, score}` rows; score = `(open − breached) / open × 100` over **open leads only** | none | `medianOfSorted` / `percentileOfSorted` | `renderRMScoreTable` (FN-079), `tab-morning.js` (`JS-020`) | reusable |
| FN-079 | `renderRMScoreTable` / `renderRMTable` `#L586/#L1017` | score rows | the RM tables; `renderRMTable` flags ±25% load vs peer average | DOM write | `computeRMScoreRows` (FN-078), `renderBreakdownCard` (FN-082) | `renderAll` (FN-077) | specific |
...
| FN-082 | `renderBreakdownCard(el, opts)` / `colorForIssue(issue, i)` `#L1457/#L1449` | element + options | a shared breakdown card / an issue colour | DOM write | `topBreakdown` (FN-083) | called back from nearly every tab file | reusable — shared helpers |
...
| FN-084 | `computeDailyLeadCounts()` / `renderDailyTrend()` `#L421/#L439` | `leads` | per-day counts / the trend chart | DOM write | `istDateKey` (`JS-005`) | `renderAll` (FN-077), `tab-morning.js` | reusable / specific |
| FN-085 | `downloadIssuesCSV()` / `downloadFilteredLeadIdsCSV()` `#L1541/#L1512` | `issueLeads` / `leads` | a CSV download | triggers a browser download | `csvEscape` (FN-086) | `#downloadIssuesBtn` (`BTN-001`), `#downloadLeadIdsBtn` (`DASH-001`) | specific |
| FN-086 | `csvEscape(v)` `#L1498` | any value | CSV-safe string | none | — | every CSV export in the app | reusable |
| FN-087 | `updateTabBadges()` / `renderJumpNav()` `#L395/#L331` | counts | tab-bar badge counts / the in-page jump nav | DOM writes | `numFromCountEl` | `renderAll` (FN-077) | specific |
