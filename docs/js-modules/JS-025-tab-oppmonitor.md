...
| **Location** | `js/tab-oppmonitor.js` (309 lines) |
| **Owner** | Snehil |
...
| FN-261 | `renderOppMonitorTab()` `#L305` | `oppMonitorPeriodRows`/`oppMonitorMonthRows`, `_renderNow` | the checklist + 2 tables in `#tab-oppmonitor` | DOM writes | `_renderOppMonitorChecklist`/`_renderOppMonitorPeriodTable`/`_renderOppMonitorMonthTable` (`#L121`/`#L202`/`#L253`) | `renderAll` (`JS-012`) | specific |
...
- **Method:** written and verified in the same session. Function list is
  ...
  delta math, row-omission on absent data, and the filter-bar toggle.
...
Not yet closed — `Record Status: Drafted`, pending a live-sheet data
check (see `TAB-009`'s Next action).
