...
| `opp_at` | datetime | first Opportunity-stage transition timestamp | **appended 2026-09-21** — rows captured before then have neither column at all. Part of the content-hash (`CONTENT_HASH_DATE_FIELDS_`, `GS-008`/`JS-018`) — a lead reaching Opportunity between two captures is a real content change, correctly triggers a new row, not a dedup skip. Read by `js/tab-oppmonitor.js`'s live same-day/48h Opp% computation via the live `leads` array — Movement_Log itself is not that computation's data source, this column is captured here purely for historical retention, same reasoning as `rm_is_active`/`lead_closing_reason` above. **Column order (2026-09-25):** `opp_at` is column 25 and `content_hash` column 26 — every writer puts all fields before the hash. From 09-22 to 09-25 the live header had those two labels swapped (the self-heal appended `opp_at` after `content_hash`), so the dedup read the wrong column and appended every lead on every run (~52k rows); repaired by hand and the self-heal fixed (`GS-008` `EXC-100`). Rows captured before 09-22 12:44 keep their hash in column 25 and have none in 26 — they age out under the 7-day retention. |
| `content_hash` | text | SHA-256 over the row's fields | Trailing column, column 26. The dedup compares a row's hash with the latest stored hash for the SAME ROW IDENTITY `lead_id|RM` (2026-09-26; was `client_id`, which several rows share, so all but one re-appended every capture). See `GS-008` FN-296 / `JS-021` FN-297. |
