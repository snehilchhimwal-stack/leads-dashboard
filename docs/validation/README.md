...
| **`js/*.js` render pipeline** | `fetchAndRender` + tab renderers don't throw on synthetic data; key outputs correct | `tests/frontend-harness.html` — **in CI headless, non-blocking** (`test/run-frontend-harness.mjs` via Playwright). A blocking attempt (`fd59944`) failed at ~28 s before install finished → **CI pass/fail not yet trustworthy**; the **local** run is the real signal. | local run 2026-09-10 (25–26 assertions clean); CI wiring exists, install path being hardened | ⚠ local-verified; CI advisory |
