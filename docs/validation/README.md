...
| **`js/*.js` render pipeline** | `fetchAndRender` + tab renderers don't throw on synthetic data; key outputs correct | `tests/frontend-harness.html` — **own `frontend-harness` CI job on `mcr.microsoft.com/playwright:v1.47.2-jammy`, blocking** (`test/serve-and-harness.mjs` → `run-frontend-harness.mjs`). Container ships chromium + all libs; the harness clock is frozen so it's deterministic (the L011 time-of-day flake, §9.7.2, is fixed). | **CI run `3675b93` — `frontend-harness` job green, 59/59** | ✅ blocking, green |
