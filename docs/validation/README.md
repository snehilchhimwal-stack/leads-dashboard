...
| **`js/*.js` render pipeline** | `fetchAndRender` + tab renderers don't throw on synthetic data; key outputs correct | `tests/frontend-harness.html` — **own `frontend-harness` CI job on `mcr.microsoft.com/playwright:v1.47.2-jammy`, blocking** (`test/run-frontend-harness.mjs`). The container ships chromium + all libs, so the ~28 s bare-runner launch failure (`fd59944`) is gone. | local run 2026-09-10 (25–26 assertions clean); CI job on the Playwright container | ✅ blocking (container job) |
