...
| **`js/*.js` render pipeline** | `fetchAndRender` + tab renderers don't throw on synthetic data; key outputs correct | `tests/frontend-harness.html` — **not yet in CI** (needs a headless browser; `t-tf-5ad22d8e4c2e` P2); run manually, `window.__harnessResults` / `HARNESS_RESULT` console line | last manual run 2026-09-10 (25–26 assertions clean) | ⚠ manual only |
