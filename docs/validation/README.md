...
| **`js/*.js` render pipeline** | `fetchAndRender` + tab renderers don't throw on synthetic data; key outputs correct | `tests/frontend-harness.html` — **in CI headless, blocking** (`test/run-frontend-harness.mjs` via Playwright) since 2026-09-10; only browser-install flakiness warns-and-skips | CI runs #81, #83–#86 — harness step green (5×) | ✅ blocking |
