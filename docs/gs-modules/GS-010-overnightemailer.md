...
| **Location** | `OvernightEmailer.gs` (2165 lines) |
| **Owner** | Snehil |
...
- `sendCombinedMorningEmail_` — added 2026-09-23: builds and sends the
  ...
  `checkpoint1_json`/`checkpoint1_sent_at` to `AllIssues_Log`. **Since
  2026-09-26 it sends nothing when the bucket has no overnight leads and
  every Checkpoint 1 lead is resolved** (state still written back; no
  `Overnight_Log` row).
...
- `sendCombinedFollowupEmail_` — added 2026-09-23 (Step 7): builds and
  ...
  `checkpoint2_json`/`checkpoint2_sent_at` to `AllIssues_Log`. **Since
  2026-09-26 it sends nothing when neither section has a still-unresolved
  lead** (Checkpoint 2 is still recorded; `followup_sent_at` stays blank).
...
`setupOvernightEmailer()` (`#L1907`) installs `sendOvernightMorningEmails`
...
| FN-235 | `pushUnresolvedToLeadFollowups_(ss, entries)` / `waitForFollowupSuggestions_(ss, leadIds)` `#L1169/#L1230` | flagged-lead entries | upserts `Lead_Followups`; **polls up to ~2 minutes for a human/dashboard-generated follow-up suggestion before falling back to the keyword engine** | Sheets write; polling read | `noCommentFollowUpGs_` / `overnightFollowupHintGs_` (`GS-005`) | FN-231 | specific — **the backend side of the same `Lead_Followups` bridge the client's Generate cycle writes into** (`JS-016` / `JS-018`); already idempotent by construction (upsert by `lead_id`, never appends a duplicate row — confirmed during the Step 8/11 idempotency audit, no code change needed here) |
| FN-236 | `sendThreadedGmailReply_(threadId, to, cc, subject, plainBody, htmlBody)` `#L1300` | a thread id + content | a reply on that Gmail thread | raw **Advanced Gmail Service** call | — | FN-232, FN-280, `backfillTodaysOvernightLogRecipientsNow` | specific — uses the raw API because `GmailThread.reply()`/`replyAll()` hard-code the recipient to "sender of the last message" (a real production bug this works around) |
...
| FN-239 | `setupOvernightEmailer()` `#L1932` | — | installs the 10:00 + 13:00 triggers; **also calls `setupRmHierarchy()`** | creates triggers; runs `GS-011` setup | `ScriptApp`, `setupRmHierarchy` (`GS-011`) | Apps Script editor (manual) | specific |
...
- **Method:** full read at `c82ec67`; function list verified by grep;
  ...
  directly from `setupOvernightEmailer()` (`#L1222`), cross-checked
...
`## Validation` citation at `#L1222`, tied to commit `c82ec67`,
...
**2026-09-26** (`87114a3`): user request "no need to send email for resolved status, only if not resolved then send email". A checkpoint email now lists only leads still unresolved: `buildAllIssuesCheckpointSectionOptsGs_` drops resolved / not_found leads (one KPI tile, "Lead(s) Still Unresolved", replaces the three-tile Total/Active/Resolved row); `sendCombinedMorningEmail_` and `sendCombinedFollowupEmail_` skip the send entirely when nothing is unresolved, and still write `checkpoint1_json`/`checkpoint2_json` (via a shared `writeCheckpointNState_` closure that also replaced the old post-send block) so the rows are not re-checked. A skipped 10:00 writes no `Overnight_Log` row (so a lead resolved at 10:00 that reopens by 13:00 has no thread to reply into and is not reported until the next 17:00 report); a skipped 13:00 leaves `followup_sent_at` blank. The Section 1 (Overnight) paths already excluded resolved leads. +25 lines (2140L → 2165L, anchors re-mapped, including some earlier drift). Tests: `Tests_OvernightEmailer.gs` (Pune lists only its unresolved lead; an all-resolved bucket sends nothing at 10:00 and 13:00 but records state), `Tests_EmailLifecycleFullCycle.gs` (lead resolved before 10:00 / between 10:00 and 13:00 -> no email), `Tests_SlaEngine.gs` (rule + filter). **Deployed live 2026-09-26** (~13:45 IST): `OvernightEmailer.gs` and `SlaEngine.gs` applied to the Sheet's Apps Script editor as exact diff edits and saved; SHA-256 of both saved files re-read in a fresh editor tab equals the committed files (`806e7930…` / `44a6a9f8…`). No trigger re-run needed (no `setupXxx()` change). First real run to watch: tomorrow's 10:00 (a bucket with nothing unresolved should send nothing and still show `checkpoint1_sent_at` set on its `AllIssues_Log` rows).
