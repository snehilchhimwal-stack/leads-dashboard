...
const OUTCOME_RULES = [
  ...
  { outcome: 'Wrong Number', signals: [
    ...
    ], test: (c, w) => (_anySignal(w, ['invalid']) && _anySignal(w, ['number', 'no']))
  ...
  // fixing this same file's mirror's de-dup bug — see
  // UnmatchedCommentLogger.gs): by far the SINGLE LARGEST recurring
  // bucket in that batch was a customer already being worked by a
  // DIFFERENT RM or channel partner — "already in touch with RM X", "she
  // already touch with someone", "already discussed with other rtmi",
  // "already spoke to someone asking details directly". Distinct from
  // Booked Elsewhere (no purchase decision implied, just a duplicate/
  // overlapping lead) and from Needs Cross-Team Routing (that's the RM
  // asking for a hand-off; this is the CUSTOMER reporting they're already
  // someone else's). Checked at the same tier, since both mean "don't
  // just retry the call — this needs de-duplication/ownership resolved
  // first".
